<template>
  <DashboardWrapper>
    <div class="sticky top-0 z-10 bg-[#f9f9f9] pt-5">
      <SectionHeader :title="title" child="Subcategories">
        <template #icon>
    ...
    <DataTable :value="filteredCategories" class="p-datatable-sm" :rows="pageSize" paginator
        ...
        <template #body="{ data }">
          <div class="flex items-center justify-center">
            <div v-if="data.logo" class="w-16 h-16 bg-gray-100 rounded overflow-hidden">
              <img :src="baseUrl + data.logo" :alt="data.name || 'Subcategory image'"
      ...
      <Column field="name" header="Nom" sortable>
        <template #body="{ data }">
          <div>{{ data.name }}</div>
          <div v-if="data.products && data.products.length > 0" class="text-xs text-gray-500">
      ...
      <template #empty>
        <div class="flex justify-center items-center py-4">
          <span class="text-gray-500">Aucun élément trouvé</span>
        </div>
      </template>
    </DataTable>
...
const title = computed<string>(() =>
  parentCategory.value
    ? `Sous-catégories de "${parentCategory.value.name}"`
...
const getCategoriesLogic = async () => {
  try {
    loading.value = true
    const result = (await getCategory(categoryId.value)).data.data
    parentCategory.value = result as Category
    categories.value = result.subCategories as IsLoadable<Category>[]
  } catch (error) {
    console.error(error)
    toast.add({
      severity: 'error',
      summary: 'Erreur',
      detail: 'Impossible de charger les sous-catégories',
      life: 3000
    })
  } finally {
    loading.value = false
...
const deleteCategoryLogic = async (category: Category) => {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Succès',
      detail: 'Sous-catégorie supprimée avec succès',
      life: 3000
    })
  } catch (error) {
    ...
    toast.add({
      severity: 'error',
      summary: 'Erreur',
      detail: axiosError.response?.data.message || 'Échec de la suppression de la sous-catégorie',
      life: 3000
    })
  } finally {
    if (existingCategory) {
...
const createCategoryLogic = async (value: UpsertCategoryDto) => {
  isAdding.value = true
  try {
    value.parentCategoryId = parseInt(categoryId.value)
    ...
    toast.add({
      severity: 'success',
      summary: 'Succès',
      detail: 'Sous-catégorie créée avec succès',
      life: 3000
    })
  } catch (error) {
    const axiosError = error as AxiosError<{ message: string }>
    toast.add({
      severity: 'error',
      summary: 'Erreur',
      detail: axiosError.response?.data.message || 'Échec de la création de la sous-catégorie',
      life: 3000
    })
  } finally {
...
const updateCategoryLogic = async (value: UpsertCategoryDto) => {
  const existingCategory = categories.value.find((cat) => cat.id === value.id)
  if (!existingCategory) return
  existingCategory.isLoading = true
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Succès',
      detail: 'Sous-catégorie mise à jour avec succès',
      life: 3000
    })
    ...
    toast.add({
      severity: 'error',
      summary: 'Erreur',
      detail: axiosError.response?.data.message || 'Échec de la mise à jour de la sous-catégorie',
      life: 3000
    })
  } finally {
    if (existingCategory) {
