  ...
  <div style="margin-top: 7vh" :class="isMobile ? '' : 'space-y-6 p-6 rounded-lg shadow-md max-w-2xl mx-auto'">
    ...
    <form @submit.prevent="onSubmit">
      ...
      <FloatLabel class="w-full mt-5" variant="on">
        <IconField>
          ...
          <InputText
            ...
            @focus="triggerValidation(v$, 'firstName')"
            @blur="triggerValidation(v$, 'firstName')"
          />
        </IconField>
        <label for="first_name" class="block text-gray-400 text-lg font-medium mb-2">First Name</label>
      </FloatLabel>
      <!-- error message -->
      <div v-if="v$.firstName.$error" class="text-red-500 text-sm mt-2">
        <span v-if="v$.firstName.required.$invalid">First name is required.</span>
        <span v-else-if="v$.firstName.maxLength.$invalid">First name must be less than 255 characters.</span>
      </div>
      <!-- Last Name -->
      <FloatLabel class="w-full mt-5" variant="on">
        <IconField>
          ...
          <InputText
            ...
            @focus="triggerValidation(v$, 'lastName')"
            @blur="triggerValidation(v$, 'lastName')"
          />
          <label for="last_name" class="block text-gray-400 text-lg font-medium mb-2">Last Name</label>
        </IconField>
      </FloatLabel>
      <!-- error message -->
      <div v-if="v$.lastName.$error" class="text-red-500 text-sm mt-2">
        <span v-if="v$.lastName.required.$invalid">Last name is required.</span>
        <span v-else-if="v$.lastName.maxLength.$invalid">Last name must be less than 255 characters.</span>
      </div>
      <!-- Username -->
      <FloatLabel class="w-full mt-5" variant="on">
        <IconField>
          ...
          <InputText
            ...
            @focus="triggerValidation(v$, 'userName')"
            @blur="triggerValidation(v$, 'userName')"
          />
          <label for="user_name" class="block text-gray-400 text-lg font-medium mb-2">Username</label>
        </IconField>
      </FloatLabel>
      <!-- error message -->
      <div v-if="v$.userName.$error" class="text-red-500 text-sm mt-2">
        <span v-if="v$.userName.required.$invalid">Username is required.</span>
        <span v-else-if="v$.userName.maxLength.$invalid">Username must be less than 255 characters.</span>
      </div>
      <!-- Phone -->
      <FloatLabel class="w-full mt-5" variant="on">
        <IconField>
      ...
      <div v-if="v$.phone.$error" class="text-red-500 text-sm mt-2">
        ...
        <span v-else-if="v$.phone.maxLength.$invalid || v$.phone.minLength.$invalid">Phone number must be exactly 11 digits.</span>
      </div>
      <!-- Email -->
      <FloatLabel class="w-full mt-5" variant="on">
        <IconField>
      ...
      <FloatLabel class="w-full mt-5" variant="on">
        <IconField>
          ...
          <InputText
            ...
            @focus="triggerValidation(v$, 'jobTitle')"
            @blur="triggerValidation(v$, 'jobTitle')"
          />
          <label for="job_title" class="block text-gray-400 text-lg font-medium mb-2">Job Title</label>
        </IconField>
      </FloatLabel>
      <!-- error message -->
      <div v-if="v$.jobTitle.$error" class="text-red-500 text-sm mt-2">
        <span v-if="v$.jobTitle.required.$invalid">Job title is required.</span>
        <span v-else-if="v$.jobTitle.maxLength.$invalid">Job title must be less than 255 characters.</span>
      </div>
      <!-- Department -->
      <FloatLabel class="w-full mt-5" variant="on">
        <IconField>
          ...
          <InputText
            ...
            @focus="triggerValidation(v$, 'department')"
            @blur="triggerValidation(v$, 'department')"
          />
          <label for="department" class="block text-gray-400 text-lg font-medium mb-2">Department</label>
        </IconField>
      </FloatLabel>
      <!-- error message -->
      <div v-if="v$.department.$error" class="text-red-500 text-sm mt-2">
        <span v-if="v$.department.required.$invalid">Department is required.</span>
        <span v-else-if="v$.department.maxLength.$invalid">Department must be less than 255 characters.</span>
      </div>
      <!-- Password -->
      <FloatLabel class="w-full mt-5" variant="on">
        <IconField>
          ...
          <Password
            ...
            toggleMask
          />
          <label for="password" class="block text-gray-400 text-lg font-medium mb-2">Password</label>
        </IconField>
      </FloatLabel>
      <!-- error message -->
      <div v-if="v$.password.$error" class="text-red-500 text-sm mt-2">
        ...
        <span v-else-if="v$.password.containsUpperCase.$invalid">Password must contain at least one uppercase letter.</span>
        <span v-else-if="v$.password.containsLowerCase.$invalid">Password must contain at least one lowercase letter.</span>
        <span v-else-if="v$.password.containsNumber.$invalid">Password must contain at least one number.</span>
        <span v-else-if="v$.password.containsSymbol.$invalid">Password must contain at least one symbol.</span>
        <span v-else-if="v$.password.containsSpecial.$invalid">Password must contain at least one special character.</span>
      </div>
      <!-- Confirm Password -->
      <FloatLabel class="w-full mt-5" variant="on">
        <IconField>
          ...
          <Password
            ...
            toggleMask
          />
          <label for="confirmPassword" class="block text-gray-400 text-lg font-medium mb-2">Confirm Password</label>
        </IconField>
      </FloatLabel>
      <!-- error message -->
    ...
    <div class="text-center mb-1">
      OR <br />
      <!-- Google Login Button (Commented out as googleSignIn service is not defined) -->
      <!-- <GoogleLogin :callback="handleGoogleSignUp" /> -->
      <!-- TODO: Implement Google Sign-In by defining googleSignIn service in auth.service.ts -->
    </div>
  </div>
</template>
...
const confirm_password = ref<string>(''); // Separate field for confirm password
...
const rules = {
  ...
  jobTitle: { required, maxLength: maxLength(255) },
  department: { required, maxLength: maxLength(255) },
  password: {
    required,
...
const v$ = useVuelidate(rules, { ...user, confirm_password }, { $autoDirty: true });
...
const onSubmit = async () => {
  ...
  if (!v$.value.$invalid && !isSubmitting.value) {
    ...
    try {
      ...
      toast.add({ life: 5000, severity: 'success', summary: 'Success', detail: 'Registration successful!' });
      // Reset form
...
// TODO: Implement Google Sign-In when googleSignIn service is defined in auth.service.ts
/*
const handleGoogleSignUp = async (response: any) => {
  try {
    ...
    if (data.token) {
      ...
      await authStore.login({ email: data.user.email, password: '' }); // Adjust based on googleSignIn response
      router.push({ name: 'HomePage' });
    }
  } catch (error) {
