                ...
                <form @submit.prevent="onSubmit">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                        <div class="col">
                            ...
                            <Password :feedback="false" toggleMask placeholder="Password"
                                class="w-full rounded-lg text-white border-gray-600 focus:ring focus:ring-green-500"
                                v-model="form.password" :invalid="v$.password.$error"
                                @focus="triggerValidation(v$, 'password')" @blur="triggerValidation(v$, 'password')" />
                            <!-- error message -->
                            <div v-if="v$.password.$error" class="text-red-500 text-sm mt-2">
                                <span v-if="v$.password.required.$invalid">Password is required.</span>
                                <span v-else-if="v$.password.minLength.$invalid">Password must be at least 8
                        ...
                        <div class="col">
                            <label for="confirmPassword" class="block text-gray-400 text-lg font-medium mb-2">Confirm
                                Password</label>
                            <Password :feedback="false" toggleMask id="confirmPassword" placeholder="Confirm Password"
                                class="w-full rounded-lg text-white border-gray-600 focus:ring focus:ring-green-500"
                                v-model="form.confirm_password" :invalid="v$.confirm_password.$error"
                            ...
                            <div v-if="v$.confirm_password.$error" class="text-red-500 text-sm mt-2">
                                ...
                                <span v-else-if="v$.confirm_password.sameAsPassword.$invalid">Passwords must
                                    match.</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="canRerequestToken" class="text-red-500 text-sm mt-2">
                        <span>
                            <Button @click="resendResetPasswordAction" link :loading="isSubmitting"
